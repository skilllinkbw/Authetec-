...
def _validate_td3_fields(f: Dict[str, str]) -> List[str]:
    ...
    # Composite (final) check digit over doc number + check + DOB + check +
    # expiry + check + optional data (RAW, fillers count as 0) + its check.
    composite = (
        f["document_number"] + f["document_number_check"]
...
def _parse_td1(lines: List[str]) -> Dict[str, str]:
    ...
    Line 1: doc code (2) | issuer (3) | doc number (9) | check (1) | optional (14)
    Line 2: DOB (6) | check (1) | sex (1) | expiry (6) | check (1) |
            nationality (3) | optional2 (11) | composite check (1)
    Line 3: name (30)

    Verified against ICAO sample: doc "D23145890" -> check 7 at l1[14];
    DOB "740812" -> check 2 at l2[6]; expiry "120415" -> check 9 at l2[14].
    Note: the TD1 composite check digit is parsed but NOT validated — the
    exact composite field coverage is not verified here (see report).
    """
...
def _parse_td2(line1: str, line2: str) -> Dict[str, str]:
    ...
    r["expiry_date"] = line2[21:27]
    r["expiry_check"] = line2[27]
    r["optional_raw"] = line2[28:35]  # raw fillers retained
...
def _validate_common_date_fields(f: Dict[str, str]) -> List[str]:
    ...
    Note: TD1/TD2 composite check digits are parsed but NOT validated —
    their exact composite field coverage is not independently verified in
    this implementation. Individual document-number, DOB and expiry check
    digits are fully validated.
    """
    issues: List[str] = []
    if not validate_check_digit(f["document_number"], f["document_number_check"]):
        issues.append("document_number check digit failed")
...
def validate_mrz(lines: List[str], expected_type: str = "auto") -> MrzValidationResult:
    ...
    if mrz_type == "TD3":
        ...
        result.issues = _validate_common_date_fields(result.fields)
    elif mrz_type == "TD2":
        if len(clean) < 2:
